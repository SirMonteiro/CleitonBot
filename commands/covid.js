  ...
  rp({ uri: url, json: true })
    .then((res) => {
      ...
      translate(res.country, { from: "en", to: "pt" }).then((pais) => {
        let casosconfirmados = res.cases
          .toString()
          .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
        let casosrecuperados = res.recovered
          .toString()
          .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
        let mortes = res.deaths
          .toString()
          .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
        let attfulldate = new Date(res.updated);
        let atualizado =
          "Atualizado em " +
          Number(attfulldate.getHours() - 3) +
          ":" +
          attfulldate.getMinutes();
        if (!res.countryInfo) {
    ...
    .catch((err) => {
      msg.edit("País não encontrado ou sem casos");
    });
};
